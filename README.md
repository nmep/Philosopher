...
## Definition / Prototype  [tous sauf mutex]
...
```
   ...
   **Suivez les etapes on va voir ca enssemble.**  
     ...
     Mais comment savoir qu'un data race a lieux ? **valgrind**, avec l'otptions **--tool=helgrind** vous verez qu'il y a une data race sur telle variable.
```bash
...
## Pilier du projet qui m'ont embete
 ...
 - **la gestion du temp**  
    elle n'etait pas parfaite [pourquoi](https://medium.com/@jalal92/the-dining-philosophers-7157cc05315) <- voir la partie sur la gestion du temps et ce qui pose probleme.  
    Pour resumer un usleep de 200 ne fera pas un usleep de 200 mais un usleep de 250 -     300, pourquoi ? parce que usleep utilise un systeme de check pour verifier si le temps a ete atteint et va check tout les x micro seconde si le temps voulue a ete atteint et ce qui finit par creer un decallage.  
...
- **Gestion des fourchettes**  
    ...
    Imaginons que tout les threads soient lance en meme temps et qu'en arrivant ils prendraient tous la fourchette a leurs droite et qu'en suite ils decident de prendre la fourchette qui est a leurs gauche, et bien vu que la fourchette a leurs gauche est la fourchette de droite du voisin de gauche et que le voisin de gauche a deja prit sa fourchette de droite, du point du philo du philo ou nous somme la fourchette de gauche est prise et on doit attendre qu'elle soit disponible pour la prendre mais tant que la fourchette de gauche est prise je ne lache pas ma fourchette de droite car une fois qu'une fourchette est prise et que je suis dans l'attente du mutex je ne peux pas executer du code donc je serais bloquer.  
      ...
      Avec valgrind ou fsanitize ou voit que "l'ordre des fourchettes a ete viole", ce qu'il faut appliquer un ordre de lock a mutex a nos thread pour eviter un deadlock ou tout pottentiel dead lock.
    ...
    Pour avoir une gestion des fourchettes qui evite les deadlock il faut appliquer une strategie au choix (vous pouvez aussi faire la votre), je vais montrer celle du cas **paire impaire**. (wikipedia)  
    ...
    **explication**  
    ...
    Ce que je conseille pour faciliter l'utilisation est de predefinir a l'avance dans un tableau de structure les index des deux fourchettes de chaque philo qui sera utiliser pour cibler le mutex de chaque fourchette, cela evite a avoir a rajouter des calculs dans le threads et de le rendre plus lent. et ne pas faire une structure trop grosse (il est plus simple de se deplacer avec un sac a dos qu'un lave linge).
    ...
    **Donc s'il y a un mutex pour l'utilisation de printf quelque part dans le code, ca doit etre le meme mutex partout dans le code et pas un autre mutex et c'est pareil pour  
    d'autre variable**
